# MyTerraria - 简化版泰拉瑞亚克隆计划

## 项目背景
Fegaria-Remastered 项目（~4500行代码，15+文件）是一个功能完备的泰拉瑞亚克隆，拥有复杂系统（XML数据驱动、精灵图、敌人AI、菜单系统、存档/读档、粒子特效、光照系统等）。我们的目标是基于其核心框架，在 `D:\Python Gaming\Terraria\MyTerraria\` 创建一个**大幅简化版本**，只实现基本玩法：移动跳跃、挥剑攻击、破坏方块、放置方块。

**核心策略：** 保留框架模式（游戏循环、delta time 物理、Tile 世界、摄像机），用硬编码数据替代 XML，剥离所有非必要系统。

**渲染策略（便于后续升级精灵图）：**
- 第一版使用纯色矩形渲染，但通过抽象函数隔离图像获取逻辑
- 后续可直接复制原项目 `res/` 文件夹，只修改以下函数即可切换到精灵图渲染：
  - `get_tile_surface(tile_id)` → 返回方块图像（当前返回纯色矩形，后续改为加载 `res/images/tiles/` 精灵）
  - `get_item_icon(item_id)` → 返回物品图标（当前返回纯色矩形，后续改为加载 `res/images/items/` 精灵）
  - `Player.draw()` → 玩家渲染（当前画矩形，后续改为加载 `res/images/` 身体/手臂精灵）
- 游戏逻辑代码无需修改，渲染和逻辑完全解耦

---

## 目标文件结构（约 980 行代码）

```
MyTerraria/
  run.py              (~30 行)    入口文件
  src/
    __init__.py       (0 行)
    constants.py      (~70 行)    常量 + 硬编码方块/物品数据
    perlin.py         (~200 行)   从原项目复制，只保留 2D 噪声
    world.py          (~200 行)   世界数据 + 地形生成 + 渲染
    player.py         (~200 行)   玩家物理 + 背包 + 方块交互
    game.py           (~300 行)   主游戏循环 + 摄像机 + 输入 + UI
```

---

## 实现步骤

### 阶段一：基础框架

**步骤 1：创建 `constants.py`** — 替代原项目的 `commons.py` + `game_data.py` + `item.py`
- 核心常量：`BLOCKSIZE=16`、`GRAVITY`、`PLAYER_SPEED=12`、`JUMP_VELOCITY=-50`、`PLAYER_REACH=8`、`FPS=60`、窗口大小 1280x720
- 世界大小：`WORLD_WIDTH=400`、`WORLD_HEIGHT=200`
- 方块字典：AIR(0)、GRASS(1)、DIRT(2)、STONE(3)、WOOD(4)、COPPER_ORE(5) — 每个含 `{name, color, solid, drop_item}`
- 物品字典：铜镐、铜剑、泥土/石头/木头/草方块 — 每个含 `{name, color, is_block, place_tile, is_pickaxe, is_sword, damage}`
- 默认快捷栏：`[铜镐, 铜剑, 泥土x99, 石头x99, 木头x99, None, None, None]`

**步骤 2：复制 `perlin.py`** — 只保留 2D 噪声函数
- 从 `Fegaria-Remastered/src/perlin.py` 复制，只保留 `noise2` 方法

### 阶段二：世界系统

**步骤 3：创建 `world.py`** — 从原项目 ~1200 行简化到 ~200 行
- `World` 类：`tile_data[x][y]` = 单个整数（去掉原项目的墙壁层）
- `tile_in_map(x, y)`：边界检查
- `get_neighbor_count(x, y)`：计算相邻实心方块数（用于放置验证）
- `generate_terrain(world)`：柏林噪声地表 + 泥土层 + 石头层 + 洞穴 + 铜矿 + 简单树木 + 出生点
- `create_terrain_surface(world)`：用纯色矩形绘制所有方块到大型 pygame.Surface
- `update_tile(surface, world, x, y)`：方块改变时重绘单个格子

与原项目的关键简化：
- 无生物群系、无墙壁、无多方块、无箱子、无建筑结构
- 纯色矩形替代精灵遮罩

### 阶段三：玩家系统

**步骤 4：创建 `player.py`** — 从原项目 ~1390 行简化到 ~200 行
- `Player` 类：position、velocity、rect、hotbar、hp、direction、swing 状态
- `update(world, dt)`：重力 + 移动 + 碰撞检测（原项目 5x5 格子检测，305-366行） + 世界边界限制
- `jump()`：如果着地则设置 velocity.y = JUMP_VELOCITY
- `use_item(world, mouse_tile)`：根据手持物品分派到挖掘/放置/挥剑
  - 镐：将方块设为 AIR，掉落物直接加入快捷栏
  - 方块物品：检查距离 + 不与玩家重叠 + 有相邻方块，放置方块，减少数量
  - 剑：触发挥剑动画
- `draw(screen, cam_x, cam_y)`：纯色矩形（身体 + 头部），挥剑时画一条线

与原项目的关键简化：
- 无精灵动画、无手臂/身体分离、无复杂旋转
- 挖掘直接加入快捷栏（无需掉落物实体）
- 无伤害数字、无粒子、无击退

### 阶段四：游戏循环 + 整合

**步骤 5：创建 `game.py`** — 从原项目 ~1463 行简化到 ~300 行
- `run(screen)`：主游戏函数
  - 初始化：创建 World、生成地形、创建地形 Surface、在出生点创建 Player
  - 循环：delta time、事件处理、鼠标持续按下、更新玩家、更新摄像机、渲染
- 输入处理：
  - A/D：移动，Space：跳跃，1-8：快捷栏，ESC：退出
  - 鼠标左键（持续按住）：使用物品，滚轮：切换快捷栏
- 摄像机：跟随玩家，限制在世界边界内
- 渲染管线：天空填充 -> 地形 Surface -> 玩家绘制 -> 方块高亮 -> 快捷栏 UI -> 血条 -> 翻转
- `draw_hotbar()`：8 个槽位矩形，显示物品颜色和数量
- `draw_health_bar()`：右上角血条

**步骤 6：创建 `run.py`** — 入口文件
- Pygame 初始化，创建 1280x720 窗口，调用 game.run(screen)

### 阶段五：完善
- 方块高亮光标（鼠标悬停方块的白色边框）
- 方块交互距离检查
- 挥剑视觉效果（约 0.2 秒的线条）
- 物品数量管理（放置减少，挖掘增加）
- 挖掘冷却（不能瞬间挖，添加小延迟）

---

## 原项目关键参考

| 用途 | 原文件 | 关键行号 |
|------|--------|----------|
| 物理/碰撞 | `src/player.py` | 221-366（移动、重力、碰撞） |
| 方块交互 | `src/player.py` | 593-790（use_item、place_block、use_tool） |
| 地形生成 | `src/world.py` | 432-636（generate_terrain） |
| 地形渲染 | `src/world.py` | 644-866（surface 创建/更新） |
| 游戏循环 | `src/fegaria_remastered.py` | 812-1462（完整循环） |
| 摄像机 | `src/fegaria_remastered.py` | 865-907 |
| 常量 | `src/commons.py` | 1-93 |
| 柏林噪声 | `src/perlin.py` | 整个文件 |

## 测试验证
1. 运行 `python run.py` — 游戏窗口以 1280x720 打开
2. 玩家出现在地形表面，正确下落并着陆
3. A/D 左右移动，Space 跳跃
4. 鼠标悬停显示方块高亮边框
5. 持有镐左键点击破坏方块，物品出现在快捷栏
6. 持有方块物品左键点击放置方块，数量减少
7. 持有剑左键点击显示挥剑动画
8. 数字键 1-8 和滚轮切换快捷栏
9. ESC 退出游戏


# ver1问题
完全的各种矩形，没有泰拉瑞亚的感觉
得先把各种工具，方块以及人物的资源弄上去

# ver1.1问题
1.人物目前就是一个紫色矩形加一个头发和脚，显然不够，我要人的全身。
2.草方块和石头方块之间总是有很多空白，不知道是不是噪声地形生成导致的。这个噪声只需要确保地面是崎岖不平的即可，先别想着生成洞穴了，地下全部填满（但不仅要有石头，还要添加一定量的铜矿和铁矿（前提是原项目也做了且res里有对应资源））
3.虽然物品栏有物品图标，但是人物拿在手上的还是没有模型，需要加上。
4.泰拉瑞亚里的树是可以穿过的，现在的版本不能

# ver1.2问题
1.人物依然没有完整显示。而且相比ver1.1基本没有变化，只不过是品红背景变成黑色背景了，依然没有显示完整人体，只有头发和脚
2.人物手持物品：目前显示的是带品红背景的图标，应该把背景扣去，且得将图标放大一些。而且目前虽然放上了图标，但我发现稿子和剑仍然存在白色的小棒子，所以人物握着的并不是物品而是这个白色的小棒子。简单来说你应该先把人物整体全部显示出来。然后使用物品（挥动剑，稿子，方块时）时应该是手“抓着”物品来挥动，你直接参考原项目这里的实现即可
3.给破坏方块和放置方块加上一定时间（现在是秒破坏）
4.加上基本的音效：破坏方块，挥动稿子，剑，走路等（前提是原项目里也有）

# ver1.3问题
1.人物依旧没有显示出来，你直接套用原项目显示人物和物品的实现
2.着地的音效去掉，另外我没有听到挥剑的音效。直接参考原项目里那些sword使用的音效
3.人物手持物品的实现改的不错，但是现在拿着的还是带品红背景的图标，我需要扣掉背景且放大后的图标（虽然res里只有带背景的资源，但是原项目里肯定有方法扣掉背景且放大，直接参考甚至照搬）

# ver1.4
1.手持物品：现在的情况是，只要选中某个物品，他就会直接显示在屏幕上，改成只是选中不显示，仍然只显示角色自身。只有当我按了左键，也即使用该物品的时候才将物品显示出来
2.添加进入游戏后先进入主界面：简单显示一个Terraria标题（用原项目的那个标题用的字体），以及play选项即可，然后按play直接进入游戏。（注意，我给res添加了/music，主界面播放the journey begins，进入游戏后播放overworldday）（进入主界面立即播放，进入游戏后等待差不多3s再放）
3.添加史莱姆怪物，然后我来测试能否对他造成伤害，暂时先设置史莱姆对人物无法造成伤害，但是依然会朝人物移动尝试攻击，只是攻击没伤害而已

# ver1.5
1.史莱姆目前几乎都是一刀死，我需要他抗住至少3剑。以及目前的死亡效果是直接消失，直接参考原项目史莱姆的死亡动画，应该会有死亡音效（和受击音效不太一样）以及爆炸的画面效果。以及每次史莱姆受到攻击都应该有音效（受击音效目前已经有了）
2.目前拿剑，人物向右的时候没问题，向左的时候，剑像是在往正下方捣，而不是从上往下挥（不知道你能不能懂我的意思）
3.剑和稿子拿在手上时的图标需要放大一些，你可以直接告诉我怎么调大小然后我来调

# ver2及以后扩展优化安排
1.添加生物群系系统，像原项目那样有雪地，沙漠等（看看除了这两个还有没有其他群系的资源）
2.添加昼夜系统，大概现实中30s就让他过完整个白天，然后晚上的话让他15s过完
3.跟我说下还有什么其他比较好的扩展项目（我不太想要加更多物品，我觉得没必要，但可以加一个枪和光源物品，这两个原项目里都有）


# ver2.1实现
1.先把三个群系做出来
2.原项目应该有背景墙这种方块吧？建议添加，这样我把地下的石头或土方块挖掉后会露出对应群系的背景墙
3.添加上面说的昼夜系统

## ver2.1问题
1.背景墙含有品红色背景，需要去掉
2.稍微调高一点银矿/铜矿和铁矿的生成几率


# ver2.2实现
1.解决2.1问题
2.添加枪和光源物品，直接参考原项目即可。同样的问题：得把图标的品红色背景扣掉，以及之前提到手持物品需要注意，这些问题不要再犯
3.添加史莱姆掉落物，凝胶和金币（前提是原项目有对应资源及实现）
另外，之后你每次写完代码不要直接启动，你直接结束对话即可，我来测试。除非你特别提及必须要你来测试的东西，否则你不要打开游戏

## 2.2问题
1.这个光源物品完全不对。现在的游戏看起来完全没有实现光照系统，即使是晚上看的也是比较清楚的，你现在的这个光源只是在晚上在周围放出很大和刺眼的光圈，观感非常差。如果你觉得实现光照系统比较复杂，那就先不做，先删除这个光源物品
2.虽然你说做了史莱姆的掉落物。但是可能因为物品快捷栏太小（最后一栏放的是光源，没有其他物品的位置了）我根本看不到物品里有没有添加凝胶或铜币。
3.为了以上，你得实现整个物品栏功能，原版的实现是：按esc时显示更多ui，包括整个物品栏，合成界面和quit按钮。你现在也要实现这个esc功能，你得确保按esc后游戏暂停（如果暂停不好做的话就算了），然后像原项目一样显示整个物品栏和quit按钮，暂时可以先不做合成界面。
4.你确实把背景墙方块的图标背景扣掉了。但是我要实现的效果（包括原项目也是这样）：是背景墙是连在一块的，而现在的是每个方块之间会隔着白色区域（也即原来的品红色背景区域），所以你只实现了扣掉背景，但是实际上应该还需要将图标放大，从而实现填满原来背景占据的空间，从而实现所有背景方块都是连在一起的样子（就跟原项目一样）

# ver2.3
1.解决2.2问题

## 2.3问题
做的不错，现在还有一个功能需要实现：实现物品的拖动，包括没用esc时快捷栏的物品应该支持互相拖动。以及打开esc后，支持仓库里互相拖动，以及仓库与快捷栏间的拖动。

# ver2.4
最大问题：如果想要拖动一个物品，必须鼠标一直按住左键直到放到另一个栏位才能松开。如果你只是点了这个物品一下，他会直接消失。实际上应该支持的是：点一下物品就默认他一直处于被拖拽状态，直到在另一个栏位上再点一次。这样就既可以支持一直按住左键，也可以支持只按两下左键

我说了，是你只要实现左键点击一个物品，就让他始终处于被拖拽状态，直到下一次点击。就可以实现我想要的效果，我要的就是这一个逻辑，看你的意思你用了两个逻辑，不需要，只需要“实现左键点击一个物品，就让他始终处于被拖拽状态即可”实在不行你就参考原项目。

# ver2.5
1.解决bug：还是物品拖拽问题，基本实现效果了，但是如果我左键点击一个物品后，又在他原来的栏位再点一次，物品还是会消失，单击别的栏位都没问题，但如果在一个栏位上单击一次再单击一次这个物品就消失了
2.实现鼠标悬停在物品上时显示物品名称，如果不会的话参考原项目

# ver3
算第三版把
后续优化目标：
1.添加真正的物品掉落系统，现在是物品直接进入我的快捷栏或物品栏，感受不到正反馈。实际应该是：破坏方块，方块会掉下来，然后人物离他一定距离以内会把它“吸过来”，同时在人物头顶显示“物品名称 数量”。包括怪物掉落物也是一样，击杀史莱姆后应该让他原地掉落凝胶和钱币，然后人物过去吸，而不是直接进入物品栏。
2.更多方向。。。我能想到的一点：目前我的这个项目只是一个类似demo的东西，还称不上一个游戏，因为他没有一个所谓的“玩法”，有什么方法可以给他添加一个玩法，让他可以正常的判定结束游戏或游戏胜利？泰拉瑞亚本身是用一个BOSS作为通关的，我们这里也许可以别的方式？例如限定时间内杀完足够多的史莱姆且不死亡即视为胜利

# ver3.1问题
1.需要添加拾取掉落物的音效，这个原项目中应该有，所有物品拾取都用这个音效即可

# ver3.2
1.解决3.1问题
2.着手实现boss那块内容

## 问题
1.我把史莱姆王的模型调大了，发现他很多时候会卡住，你检查下是为什么。
2.就算模型没有调大，这个史莱姆王还是有点笨了，你需要提高它的机动性，例如增大跳跃能力等等

# ver3.3及以后优化方向
1.合成系统？
2.最新想法，能否做成游戏进化那种随着游戏进程进化的？例如能否在打败史莱姆王后进化成3D游戏？
你评估一下只用目前的资源，能否把现在的游戏场景3d化（或伪3d化，就像毁灭战士那样，虽然我这个是pygame为框架，但是好像可以通过opengl什么的来引入3d技术）

# ver4引入3D后
## 问题
1.进入3D世界后，只看到了史莱姆王的贴图，其他所有都能看到形状，但是完全没有颜色，都是黑色的
2.进入3d后，上方的文字上下颠倒。我的操控也是上下颠倒，按s键往前，w往后。鼠标往上视角向下，鼠标往下视角向上

## 其他问题
1.一直对着我的那个史莱姆王的贴图“沉在地里了”，只有不到一半漏了出来
2.部分裸露出来的方块侧面是透明的，观感很不好
3.建议给3D模式单独加一个程序用来启动，或用原来的启动程序加一个参数什么的。总之我测试的时候每次都用打败史莱姆王来进入有点麻烦

# ver4.1
## 问题
1.用--3d启动后，会因为鼠标无法切出游戏框从而无法将我电脑的输入法改为英文（之前没加入3d之前我都是打开游戏后手动把输入法从中文调为英文的）
不知道你有没有方法能让中文输入法下也能正常进行键盘操作，或给3d场景下加一个暂停功能，从而能让我esc暂停后能让鼠标移出游戏框从而调整输入法
2.史莱姆王那个贴图还是低了，还得升高，你可以直接告诉我在哪里调整
3.方块的侧面透明情况依然存在，而且我发现这问题似乎没法解决。因为你似乎本来就没有那种我的世界那样的方块，你只有二维的贴图，那只有一个办法，假设一个立方体，然后把他的六个面全部加上贴图，但是你现在似乎没有这么做
4。如果我后续像做成我的世界那种真正的方块世界，以及在3d世界中不再只是“摄像机”，而是可以行走和挖掘，放置，攻击，可行性如何，用目前pygame+opengl的技术栈可以做吗
5.还有一个问题，3d情况下上方的文字变成一行白色了，不知道为什么


# ver4.2
1.还有一个问题，3d情况下上方的文字变成一行白色了，不知道为什么
针对提出的规划:
能否直接用同一个贴图作为top,side,bottom?尽量简单化,如果不行,我会考虑直接找Minecraft的原版资源
你说的moderngl和什么的,区别主要在哪,你推荐哪个,我可以不选择pythongl
难点/瓶颈部分,我本来就不做无限世界,跟一开始2d的世界大小格数差不多即可
3d怪物可以先不做,放一个静止的怪物在那里让我测试攻击功能即可

可以先分步实现,例如第一步先方块世界:我不懂你说的VBO什么的什么意思,我说的方块世界的意思就是看着像Minecraft那样真的是一个个方块,而不是现在这种贴图,我以为只需要把top,side,bottom贴图全部加上即可?那你就先都用同一个贴图,本来Minecraft大部分方块都是六个面都是相同的

# ver4.3
真不错,真的在方块世界里行走了

## 问题
1.目前无法在疾跑的过程中跳跃
2.为2d和3d的暂停模式下,添加quit按钮,按Q不是一个特别方便的选项
3.另外,贴图透明的情况依然存在,几乎没有改善.大部分因为高低差露出来的更高的那些方块的侧面就是透明的,只有少数不是.我不知道你代码写的是不是六面都有贴图,至少实际情况就是我看起来只有几面有贴图

1.实现准星选中方块 + 左键挖 / 右键放。


# ver4.4
## 问题
1.独立的方块正常,只要有至少两个连在一起,两个方块就都会有某一个测面变成透明
2.我之前说疾跑不能跳跃是因为我ctrl+空格是换输入法,所以不行,你加一个双击前进键w也可以疾跑,或者在加一个按键控制疾跑,例如shift alt之类的

# 备忘录
  已完成

  - 行走 + 重力 + 跳跃 + 疾跑（Shift / 双击 W）
  - 挖方块 / 放方块 + 准星线框高亮 + 数字键 1-6 选方块
  - 方块透明 bug、HUD、暂停/QUIT 按钮、独立启动(--3d)

  下一步（你之前明确要的）

  - 静止怪物 + 攻击测试：放一个 King Slime（或小史莱姆）当靶子，左键攻击有伤害判定 +
  血条，打死播放死亡。这是你说的“3d怪物先不做，放一个静止的怪物测试攻击”。

  之后可选方向（之前提过 / 自然的延伸）

  1. 3D 背包 + 方块数量：现在挖/放都是无限的，可以接上物品数量管理。
  2. 合成系统（你 ver3.3 提过“合成系统？”）。
  3. 3D 的目标/胜利条件：给 3D 模式一个玩法闭环。
  4. 更多方块 / 简单昼夜：丰富观感。

  要不要下一步就做静止怪物 + 攻击？还是先做别的（比如先把方块数量/背包补上，让挖放有成本）？
  ⎿  Invalid tool parameters


# ver4.6
1.在现有资源下,能否做出一个类似minecraft那样空手就展示手的模型,拿稿子展示稿子,拿剑展示剑.并且也有挥动,挖掘动画得等等?(如果现有资源不够,我可以尝试直接拿minecraft的资源)
2.现有资源下,能否直接做类似minecraft那样的3d怪物?(感觉大概率不行,我会尝试找资源)

## 问题
1.根本不是我想要的minecraft的手持物品效果,这只是在屏幕的右下角放了这个物品的图标并且让他在我按左键的时候旋转.也许我要的效果必须得用mc的资源才行?
2.以及不知道为什么现在方块可以直接悬空放置(正常来说一个方块必须放置在其他方块的某个面上,不可能在周围什么东西都没有的情况下防止在某个地方)
3.先做一个静止怪物攻击测试吧



# 3d问题汇总
1.我感觉很难做出像mc那样有“层次感的”3d模型出来，就像最开始那样6个面各放一个正方形贴图的形式挺好的，就做史莱姆怪物就好了。但是之前那个史莱姆他用的六个面都是一样的，至少要加一个眼睛，然后让他像2d里面那样能跳。但是注意：3d场景下这个史莱姆的水平位移和速度不能太快

# ver4.7
## 问题
1.跳跃做的不错,但是模型做的还是有问题,这次的除了头顶那个面,其余面都非常暗,然后也没有看到脸,不知道是解析错了那个slime.png还是他原本就没有这个脸,我又往3dres/里添加了一个slime_face.png,实在解析不了那个展开图的话,就直接用这个face来作为一个侧面就行.然后其余侧面不要用那个很暗的,用再之前那个版本(六个面都完全一样的那个)

## 后续实现
1.扩大地图,目前的3d场景太小了
2.像2d那样,添加挖掘的耗费时间,目前挖掘是瞬间完成的.
3.目前的地层还是很浅,感觉至少得有个十层吧,然后得像2d那样添加矿物
4.目前这个史莱姆有点大了,这个只是普通史莱姆的模型,不应该这么大,你可以告诉我在哪里调整
5.我跟你说的史莱姆不要一直脸对着我,但是现在是完全不会转向.我要的效果是史莱姆只会往它头朝向的地方跳,然后跳的过程中不要转向,可以在落到地上后转向我,然后再跳


# ver4.8
## 问题
1.好吧我错了,其实史莱姆一直朝向玩家也是可以的(包括跳跃过程中也允许转向),只是之前那个有点视角太锁着我了,我想要的效果是他确实会锁定我,但是不能让我感觉它的视角像是在跟我的视角相对静止,需要一点点滞后感你懂我意思吗
2.连续点鼠标左键似乎会报错(terraria) PS D:\A Python Gaming\Terraria\MyTerraria> python .\run.py --3d
pygame 2.6.1 (SDL 2.28.4, Python 3.13.12)
Hello from the pygame community. https://www.pygame.org/contribute.html
Traceback (most recent call last):
  File "D:\A Python Gaming\Terraria\MyTerraria\run.py", line 18, in <module>
    scene3d.run_epilogue(font)
    ~~~~~~~~~~~~~~~~~~~~^^^^^^
  File "D:\A Python Gaming\Terraria\MyTerraria/src\scene3d.py", line 859, in run_epilogue
    if not lmb_held or target_block is None or tuple(target_block) != tuple(mining_target):
                                                                      ~~~~~^^^^^^^^^^^^^^^
TypeError: 'NoneType' object is not iterable


3.我说的所谓的滞后一点,我认为可以就理解为,当玩家移动后,史莱姆稍等一会(1s以内),然后再开始转向我
4.目前左键按住不动不能连续挖方块,必须松开然后再次对准方块按住左键才行
5.地图还可以再大点,以及可以生成更多史莱姆(大小不同).以及目前这个世界有一点荒凉,不知道能不能生成结构?(一个简单的小木屋即可,然后里面放箱子,你看是否有对应资源)
6.目前只有物品栏,需要加入背包(按e打开)

# ver4.9（实现）
1. **地图扩大** `WORLD_SIZE = 120`, `MAX_H = 18`;`_build_terrain` 重写为 BASE_SURFACE=10 起伏,地下填满 y=0..h,散布铜矿/银矿
2. **挖掘耗时** `TILE_MINE_TIME`（草/土/沙/雪 0.3s,木 0.5s,石 0.8s,铜/银 1.1s）;`mining_progress` 累计 + 方块上方进度条
3. **多只史莱姆** `Slime3D` 类,5 只群落 (`SLIME_SPAWNS`),大小 0.6 / 0.8 / 1.0 / 1.5,各自血量
4. **滞后转向** `player_pos_lagged` 指数平滑 (`SLIME_LAG_TIME = 0.5s`),史莱姆读滞后位置算朝向 → "等一下再转"效果
5. **木屋结构** `_place_house()` 程序化生成:清空 + 木墙 + 木顶 + 门缺口 + 内嵌箱子方块
6. **连续挖掘** LMB 按住时,挖完一格自动续上准星新指向的方块 (无需松开重按)
7. **背包 UI (E 键)** 8×4 = 32 格 stash + 半透明遮罩;单击进入拖拽态,再次点击放下;支持热栏 ↔ 网格 同种合并/不同种交换/拖空白退回原位;`_stash_or_drop` 自动归位

## 问题
1.箱子需要能右键交互打开,里面要摆东西,最好是比较珍贵的物品
2.我得自己去找一下史莱姆生成的各种参数,把速度调慢一点,同时存在数目也调低一点
3.物品栏除了稿子和剑,其他方块的图标不知道为什么都是填满了格子的涂色,你直接放图标不就行了吗(我拖动方块发现显示的是图标),这个结果就导致,我把物品栏3位置的土方块放到背包后,三位置还是黄色的涂色,然后我想把土方块放到位置4发现,他自动跑到位置3去了

# ver5.0（实现）
1. **箱子右键交互** 右键瞄准 SLOT_CHEST 方块开箱子界面；每个箱子按坐标 seed 生成固定 loot（必给铜矿/银矿/木头，随机额外的石头/沙/雪等）；拖拽可在箱子 ↔ 玩家热栏/背包间自由移动；E/ESC 关闭
2. **物品栏去类型化** 把 `inv_counts`（每类方块的计数数组）换成 `hotbar`（8 槽自由栈：0/1 工具，2-7 任意方块栈）；空栈不画图标；挖掘/放置/手持全部读 `hotbar[sel_idx]`
3. **真实图标** 热栏方块槽画 `res/images/items/*.png` 真实图标（不是涂色块）；PLACEABLE_BLOCKS 扩到 8 种（新增铜/银矿石，从 chest 出）
4. **Simplify pass** 提取 `_blit_textured_quad` / `_blit_text` / `_draw_drag_icon` / `_build_grid_surface` 共用辅助；文本贴图带 `_text_cache` 避免每帧 `glTexImage2D`/`glDeleteTextures`；magic 数字换成 `_BI_COPPER = PLACEABLE_BLOCKS.index(SLOT_COPPER)` 等命名常量

# 史莱姆调参位置（ver4.9 用户问）
- `SLIME_GRAVITY_3D = 18.0` 下落加速度（越小越漂浮）
- `SLIME_JUMP_VY = 7.5` 起跳向上速度（越小跳得越低）
- `SLIME_HSPEED = 4.0` 跳跃水平速度（越小越慢）
- `SLIME_JUMP_INTERVAL = 1.0` + `SLIME_JUMP_JITTER = 0.5` 落地到下次起跳的间隔
- `SLIME_LAG_TIME = 0.5` 朝向滞后时间常数
- `SLIME_TURN_SPEED = 8.0` 朝向插值速度
- `SLIME_SPAWNS` 列表（在 `run_epilogue` 里）每条 = (rx, rz, size, hp)，删/加条目即可改数量


# ver5
## 问题
1.物品栏里的物品图标做好了，但是背包里的物品图标还是之前的问题，同样的道理修改
2.防止方块后明显会卡一下，你看下为什么
3.鼠标悬停到物品上时，需要显示该物品的名称